...
# ==============================================================================
SYMBOL = "UAIUSDT"# Le tue 3 Size personalizzabili (Modificabili a mano in base al prezzo)
QTY_LIVELLO_NORMALE = 100  # Size standard con mercato tranquillo
QTY_LIVELLO_ALTO = 50     # Size ridotta con mercato nervoso
QTY_LIVELLO_ESTREMO = 20   # Size minima di emergenza con mercato impazzito# SOGLE DI ATTIVAZIONE (In salita - Calcolate sulle 24 ore mobili)
SOGLIA_ALTA_VOLATILITA = 25.0    # Sopra il 20%, passa a size 50
SOGLIA_ESTREMA_VOLATILITA = 50.0  # Sopra il 40%, passa a size 20# SOGLIE DI RIPRISTINO / RIENTRO (In discesa per evitare l'effetto altalena)
RESET_DA_ALTO_A_NORMALE = 18.0   # Torna a 100 solo se scende sotto il 15%
RESET_DA_ESTREMO_A_ALTO = 35.0   # Torna a 50 solo se scende sotto il 30%# 8 Livelli: il bot moltiplicherà la tua BASE_QTY attuale per questi coefficienti
GRID_MULTIPLIERS = [1, 1, 1.1, 1.3, 1.5, 2.4, 2.7, 2.9]# Calibri degli spaziatori per arrivare precisi al 16% di calo cumulativo
GRID_SPACING = [0.0, 0.8, 1.0, 1.2, 1.5, 3.0, 4.0, 6.0]# Target profit fisso della griglia (1% sul prezzo medio ponderato)
TAKE_PROFIT_PERCENT = 1# Distanza percentuale dello Stop Loss Fisso dal prezzo di partenza dell'L1
STOP_LOSS_PERCENT = 21COOLDOWN = 30  # Secondi di pausa dopo la chiusura di una griglia# ==============================================================================
# DECIMALI STRUMENTO
# ==============================================================================
PRICE_DECIMALS = 5
QTY_DECIMALS = 0 # ==============================================================================
...
prezzo_inizio_griglia = 0.0 # Livello di rischio corrente: "NORMALE", "ALTO", "ESTREMO"
stato_rischio_attuale = "NORMALE"  
BASE_QTY = QTY_LIVELLO_NORMALE   # Inizializzazione iniziale# Connessione alle API di Bybit
session = HTTP(
    testnet=False, 
...
def check_daily_report():
    ...
    if nowdt.strftime("%H:%M") in ("06:00","18:00") and key!=ultimo_report:
        ...
        if bal is not None:
            asyncio.run(telegram_send(f"📊 REPORT BOT\n\n💰 Saldo USDT: {bal:.2f}\n🪙 {SYMBOL}\n🕒 {nowdt.strftime('%d/%m/%Y %H:%M')}"))
            ultimo_report=key
...
def round_price(price):
    return round(price, PRICE_DECIMALS)def round_qty(qty):
    if QTY_DECIMALS == 0:
        return int(round(qty))
    return round(qty, QTY_DECIMALS)def get_daily_volatility():
    """
    ...
    print(f"  Errore nel recupero della volatilità Rolling 24h: {e}")
    return 0.0def cancel_all_orders():
    try:
    ...
    return Falsedef get_current_price():
    try:
...
print(f" Stop Loss Fisso (Nativo): -{STOP_LOSS_PERCENT}%\n")while True:
    try:
        check_daily_report()
        now = time.time()
        price = get_current_price()    pos_data = session.get_positions(category="linear", symbol=SYMBOL)["result"]["list"][0]
    ...
    # ==================== GENERAZIONE STRUTTURA GRIGLIA ====================
        ...
        if stato_rischio_attuale == "ESTREMO":
            ...
            print(f"  [RISCHIO: ALTO] Volatilità 24h mobili al {daily_vol:.2f}% (Soglia > {SOGLIA_ALTA_VOLATILITA}%)")
        ...
        try:
            ...
            print(f"  Errore critico nell'inserimento dello Stop Loss nativo: {sl_err}")
